...
#   1) Codex CLI を最新化（gpt-5.6-terra は新しいCLIでないと 400 で弾かれる）
#   2) PATH上の「古いcodexの影」を退治（npm prefix 外の孤児インストールは
#      `codex update` が届かないまま PATH 先頭に居座り、tmux/ログインシェルだけ
#      旧版が起動して gpt-5.6-terra が 400 で落ちる。2026-07-17 実機で遭遇）
#   3) ~/.codex/config.toml の既定モデルを gpt-5.6-terra にする（top-level `model`）
