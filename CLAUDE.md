...
**モデル側の緩和策**（事後検知フックの補完として）:
- ツールを呼び出す直前に、呼び出し自体が実行されるかを意識する（テキストと混在しないよう、ツール呼び出しは単独のターンで行う）
- ツール結果がない／前のツール呼び出しが実行された確認がない場合は、呼び出しが漏洩した可能性を疑い再試行する
- 同じ呼び出しが2回以上漏洩する場合はコンテキストが汚染されている可能性が高い → ユーザーに `/clear` を推奨する

**Stop フック**: `detect-leaked-toolcall.sh`（Linux）/ `.ps1`（Windows）が漏洩を事後検知し exit 2 でハーネスに続行を促す。
