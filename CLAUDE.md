...
エージェントを「指示する人」から「指示するシステムを設計する人」に移すとき、生成はほぼ無料になり**判断（judgment）だけが希少資源**として残る。ループの難所はループを作ることではなく、**中に "No" と言えるものを置くこと**。自走ループ・自動化・スケジュール実行・`/loop`・`/goal` を設計/構築/レビューするときは **loop-engineering** スキルを、自分（生成器）が自分の出力を採点しそうなときは **loop-evaluator** スキルを必ず読み込んで使う。

恒久姿勢として、以下の「4つの静かな負債」への防御を標準実装とする（いずれもループ稼働中は警報を鳴らさず、互いに増幅し合い、一斉に表面化する）:

1. **Verification debt（検証負債）** — 自分が書いたコードを自分で採点しない。生成と判定を構造的に分離し、独立した評価者（別エージェント／別モデル／Codex 委譲、`.claude/agents/loop-reviewer.md`）に「壊れている前提で・読むのでなく動かして」検証させ、停止判定は新鮮なモデルに委ねる。「テストが緑」「自分が書いたから把握している」「小さい diff」「N回成功したからチェック不要」は全て採点を甘くする合理化であり、出てきたら独立評価を呼ぶ合図。
2. **Comprehension rot（理解の腐敗）** — ループ出力を毎回**サンプルだけ必ず読み**、各変更を「何を・なぜそうしたか」自分の言葉で説明できるか確かめる。説明できない＝自分の地図がコードに後れた精密な信号。
3. **Cognitive surrender（判断の放棄）** — ループは**実行はできるが決定はできない**。最低1つ人間チェックポイント（一時停止する扉）を残す。楽するためでなく、いつでも中に入って止められる立場を保つため。
4. **Token blowout（トークン暴走）** — 無人稼働させる**前に**ハードな上限（1回あたり／1日あたり／最大リトライ）を設定する。「まだ暴走していないから後で」は、コストが検知前に発生し終えるため構造的に成立しない。上限は安価・可逆、暴走は青天井・不可逆。
