...
- **codex-review** — Codex CLI + codex-plugin-cc でコード・文章をレビュー。「Codexにも確認」「Codexでレビュー」「Codexに反論してもらって」で使用。git diff・ファイル単体・敵対的レビュー・構造化JSON出力・バックグラウンドジョブ対応。`/codex:*` コマンドも直接使用可。モデルは `~/.codex/config.toml` で管理（現在: `gpt-5.4`）。
