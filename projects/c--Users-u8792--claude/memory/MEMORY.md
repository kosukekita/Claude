...
- [feedback_u8792_path_unicode_escape.md](feedback_u8792_path_unicode_escape.md) — バックスラッシュ絶対パス C:\Users螒 はツール引数中で 螒 がUnicodeエスケープ解釈され C:\Users螒 に化け、静かに別の場所へ読み書きする。ツールのpathは常にフォワードスラッシュ C:/Users/u8792/... で書く
- [feedback_settings_mojibake_path_causes_api400.md](feedback_settings_mojibake_path_causes_api400.md) — settings.local.json の additionalDirectories 等に化け漢字 螒(U+8792) を含むパスが混入すると、そのプロジェクトで起動＋任意入力するだけで毎ターン API 400 invalid high surrogate。設定は毎回読み直され /clear で直らず、設定からエントリ削除で解決。孤立サロゲートだけ探すと螒(正規漢字)を見落とす
