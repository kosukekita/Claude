...
- [feedback_settings_mojibake_path_causes_api400.md](feedback_settings_mojibake_path_causes_api400.md) — 設定/記憶ファイルに化け漢字(U+8792)の生文字が混入するとSessionStartで毎回注入され、そのプロジェクトで起動＋任意入力するだけで毎ターン API 400 invalid high surrogate。固定要素は毎回読み直され /clear で直らず、生文字の除去で解決。孤立サロゲートだけ探すとU+8792(正規漢字)を見落とす
