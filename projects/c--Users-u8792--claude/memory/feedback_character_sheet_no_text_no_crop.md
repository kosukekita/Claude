...
**適用範囲の限定（ユーザー確定 2026-07-09）:** 上の2ルールは**参照画像を使う経路にだけ適用する** = テンプレートの (B) SFW→NSFW 派生・(C) 既存画像からのシート化。**参照なし t2i の (A) ペルソナ新規 SFW シートには適用しない**（日本語短ラベルを入れたまま）。理由: ルール1の Why（文字化け・英語混入・切り出し元として不要）は Qwen-Image-Edit / FLUX.2-dev / Grok の弱さに由来する参照あり経路の事情で、(A) は Codex(GPT Image) が画中日本語を正確に組める。稼働中の `~/media-out/sheet-factory/daily_sfw_sheet.mjs`（毎朝 9:00 の無人生成）もラベルありのまま。この限定は Win/Linux の履歴分岐（Win=07-07 に本ルール、Linux=07-08〜09 に (A) を実機で作り込み）をマージする際にユーザーが判断したもの。

**How to apply:**
- 正本は video-media-studio の `reference/character-sheet-template.md` 冒頭「★固定ルール」節（(B)(C) 内では本節が他記述より優先。汎用テンプレ内の「日本語ラベルのみ入れる」は上書き無効化済み）。
- 検証で「文字が入っていないか」を必ずチェック（入っていたら再生成）。ただし (A) は逆に「日本語ラベルの文字化け」を見る。
