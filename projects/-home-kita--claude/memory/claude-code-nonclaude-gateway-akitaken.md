...
- モデル一覧は `litellm-models`。セッション中は `/model <名前>`（ゲートウェイ配下では任意文字列が無検証パススルー — 公式仕様）。
- 稼働中: local-gptoss / local-qwen3 / local-qwen3-unc（ollama・無料・num_ctx は litellm の extra_body.options.num_ctx で拡張済み・検証済み）＋ atlas-gpt-5.6-sol / atlas-gpt-5.5 / atlas-grok-4.5 / atlas-glm-5.2 / atlas-kimi-k3 / atlas-deepseek-v4（従量）。
- サブスク枠ブリッジ = CLIProxyAPI（127.0.0.1:8317, `cliproxyapi.service`）。OAuth ログイン（-codex-login / -xai-login）は**エージェント実行不可（権限クラシファイアが遮断）**、ユーザー本人が実行 → 8317/v1/models を enumerate して litellm config に追記。
