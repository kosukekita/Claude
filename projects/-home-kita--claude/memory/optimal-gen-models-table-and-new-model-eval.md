...
## ★体型/人物バリエーションはプロンプトの数値でなくseedで振る(実証 2026-06-28)
プロンプトに「身長158cm/体重55kg」等の**数値を書いても、Z-Image・Chroma(拡散モデル全般)はほぼ無視**して体型に反映しない(定性記述「胸が大きい」が支配的)。BMI 22→27の4セットを同一seedで生成したら全部ほぼ同じ体型だった。→**体型/人物/構図のバリエーションが欲しいときは、プロンプトを固定して `--seed` を変える**(seed違い=別人・別体型・別構図)。数値スペックでの体型制御は効かないので時間の無駄。サブエージェントでBMI計算させるのも(数値が効かない以上)不要。今後「体型を変えて」「バリエーション」は seed を4種類振る。
...
## ★★ユーザー恒久ルール(2026-06-28訂正): 動画生成のプロンプトを勝手に変えない・変えるなら必ず事前確認
**動画生成では、ユーザーが書いた動きプロンプトを基本そのまま使う。AIが設定に無いものを勝手に足さない**。特に**「顔を見やすくする/顔をはっきり」系は設定に存在しないので絶対に勝手に出さない**(実例: 勝手に足したら逆に動画の質が落ちた 2026-06-28)。**プロンプトを変更/追記したい場合は、生成前に必ずユーザーに確認して許可を取る**(無断で変えない)。ユーザーが「1-3すべて大事」と明言=①原文尊重 ②勝手追記しない ③変えるなら確認、の3点セット。
- これは**動画のみ**。**画像生成は現状維持**=タトゥー拒否ネガ(`tattoo, tattoos, body ink, lettering on skin`)+品質ネガティブを自動適用してよい(画像はこれで問題なかった)。
