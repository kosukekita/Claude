...
- 2段ゲート: (1)relevance=画像/動画生成のみ(uncensored LLMを除外) (2)notability=likes≥10 OR dl≥200 OR trendingScore≥5。例外バイパス=主要familyのbase新規 / 明示NSFWで僅かでもtraction(likes≥1 or dl≥10)。閾値は env `HFW_MIN_LIKES/HFW_MIN_DL/HFW_MIN_TREND` で調整可。これで2日719件→7日137件に
- trendingScoreはlikesと連動するので `>0` だと「1like=通知」になり緩い→ `≥5` が「きつめ」の実体
