...
## 追記（2026-09-05）: 委譲先はサブエージェント任せにしない

Codex が使えない状況（バージョン不足＋利用上限）を把握していたのに、実装の委譲先を
サブエージェントの判断に委ねた結果、**サブエージェントが自分で Codex に投げて3秒で失敗**し、
「バックグラウンドで進行中」と誤報告してきた。私はそれを裏を取らずに受け取りかけた。

**恒久対策**:
- 委譲先は**親（私）が指定する**。サブエージェントのプロンプトに
  「実装は自分でやること／Codex に再委譲しないこと」を明記する
- サブエージェントの「バックグラウンドで進行中」は**主張であって証拠ではない**。
  `codex-companion.mjs status` 等で実体を確認してから受け取る
- ユーザー指示（2026-09-05・更新）「**codexに委譲してもいいけど、レート制限などで使えなかったら、
  すぐに fable5.1 に移転する**ようにしてください」
  → Codex を試すのは可。ただし**失敗したら即座に Fable 5.1 へ切り替える**。
  失敗の代表例: 利用上限（`You've hit your usage limit`）／
  CLI バージョン不足（`requires a newer version of Codex`）／ジョブ登録失敗（status に現れない）
  → サブエージェントに委譲を任せる場合は、**このフォールバック手順をプロンプトに明記**する
  （「Codex が失敗したら自分で実装せよ」と書かないと、失敗を『進行中』と誤報告してくる）
