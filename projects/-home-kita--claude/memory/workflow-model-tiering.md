...
metadata: 
  ...
  modified: 2026-08-09T04:45:47.025Z
---
...
**この型は保存済みコマンド化してある**: `/find-verify <調査対象>`（`~/.claude/workflows/find-verify.js`、
2026-08-09 実走検証済み）。探索4観点=sonnet → 重複排除 → 1件につき検証2体=セッションモデル（同数割れは
棄却せず未確定）→ 統合1体。計13体・約78万トークン・約2.5分。実測で**探索を2観点から4観点に広げたら
本命を取りこぼさなくなった**（2観点版は唯一の設定箇所を両方の finder が見落とし、Opus の検証側が
棄却理由を潰す過程で発見した）＝**幅は finder の数で買う／判断は絶対に安くしない**の実証。
保存済みワークフローは**起動時に読み込まれる**ので、作った直後のセッションでは `/名前` で呼べず
`scriptPath` 指定でのみ実行できる。
