...
★★★ハードゲート（重大ルール・CLAUDE.mdにも明記）: メディア生成は常に**最高品質が既定**。速さのための劣化（解像度↓ / steps↓ / フレーム間引き / 量子化・軽量モデル化 / その他あらゆる速度目的の設定）を**ユーザーの事前許可なく絶対にやらない**。速さを優先すべきと判断しても勝手に決めず、**まず「速さのためにこう下げてよいか」を尋ねて許可を得てから**行う。長時間でも許可が無い限り品質を削らない（systemd-run＋段階配信で回す）。**この種の判断（解像度やstepを下げよう等）が頭をよぎったら、実行前に必ず一旦止めてユーザーに聞く。**

★違反履歴（繰り返している＝要自戒）: 2026-07-13 長谷川バーテンダー画像で、参照ネイティブ832×1216 → **704×1024へ解像度を勝手に低下＋40→30stepを勝手に削減**（速度目的）→ ユーザー「重大な違反」。ルールは既にこの記憶にあったのに従わなかった＝ハードゲート化した理由。
