...
### ★動画生成の前に必ずストーリーボードを作り、承認を得る（例外なし）
**動画（t2v / i2v / r2v / v2v いずれも）を生成する前に、必ず次を行う。**「素早い1本」「テスト」でも省略しない。

1. **各モデルの"生成可能尺"でクリップ単位に割る**（i2v 1本＝1クリップ）。実測(AtlasCloud schema): Kling `image-to-video`=`duration`整数**3〜15秒**、Seedance `image-to-video`=**4〜15秒**（両者とも**最長15秒**）。**0.6s等の細切れクリップは作れない**。尺はこの範囲のクリップを**繋いで**作る。
   - ★**ストーリーボードの隣接キーフレーム間＝i2v 1クリップ**。よって隣接フレームの**時間差は最短尺（Kling 3s / Seedance 4s）以上・15秒以下**にする。**N枚のキーフレーム＝N-1本のクリップ＝最短 (N-1)×最短尺**。例: 開始/3s/6s/終了 の**4枚(2×2)＝3クリップ＝最短9秒**。**1.7s 等の3秒未満の間隔にフレームを置けない**（i2vの境界にできない）。中間フレームを増やすほど動画は長くなる。
2. **ストーリーボードを"1連続ショットにつき1枚の画像"として作る**（`storyboard_<shot>_NN.png`）。1枚に入れてよいのは**その1連続ショットのキーフレームだけ**（最小＝開始/終了の2枚。動きを細かく制御するなら 0s/3s/6s/… と**最短尺以上の間隔**で中間キーフレームを足した N 枚。隣接ペアが各1クリップ）。記号・矢印で誤魔化さず、**そのまま i2v の開始/終了画像に使える"実画像"**にする。同 basename の `storyboard_<shot>_NN.txt` に、**各キーフレームの時刻(秒)・各クリップの尺・総尺・内容・カメラ**を紐づける。
3. **ユーザーに提示して承認を得る。承認前に動画を生成しない**（静止画＝キーフレーム素材の生成は続けてよい）。
4. 承認後、**隣接コマを i2v の「開始画像」「終了画像」に指定**して各クリップを生成し（Kling=`image`+`end_image` / Seedance=`image`+`last_image`）、**ffmpeg concat で繋ぐ**。
5. **1ストーリーボード＝1連続ショット（＝1本以上の"連鎖クリップ"）。カメラのカット・場面転換・時間ジャンプを1回でも内包しない。** ★区別を厳守: **中間キーフレーム**で区切った複数クリップ（0s/3s/6s/…、各≥最短尺）は**同一ショット＝同一ボードでOK**（クリップnの終了＝クリップn+1の開始で滑らかに連鎖）。一方**カット（場面転換）**は別ショット＝**別ファイルの別ストーリーボード・別動画**にし、最後に`ffmpeg concat`で繋ぐ。★複数カットが同じ作品/同じ流れでも、「承認しやすい」「1つの連続した流れだから」を理由に**1枚にまとめない**（＝letter vs spiritの禁止。**カットを1枚に束ねた瞬間このルールを破っている**）。**機械的自己チェック**: 1ボード＝カット0の1連続ショット。中間キーフレームによる複数クリップは可。**場面（場所/照明/構図の飛び）が変わったら別ボード**。命名 `storyboard_<shot>_NN.(png|txt)`（連続ショット毎に1ファイル）。
6. **タイミングをフリーズフレームやスロー水増しで"それっぽく"作らない**（「途中で止まってるだけ」に見える）。動きは実生成クリップで作る。freeze/reverse等の特殊時間演出が要る場合は**VFXレイヤー(3D/合成)で別途**作り、この本編クリップと混同しない。

**Red flags（どれも「STOP・先に正しいストーリーボードを作れ」）**: 「0.6s刻みのi2v」「start/end無しでi2v」「1本にカットを詰める」「freeze-frameで尺を埋める」「承認をあとで取る」「複数カットを1枚のボードにまとめる」「1つの流れ/同じ作品だから1ボードでいい」「承認しやすいから1枚に」。

| 言い訳(rationalization) | 現実 |
|---|---|
| 「1つの連続した流れだから1ボードでいい」 | 流れ≠1ショット。カメラのカットが入れば別ショット＝別ボード。 |
