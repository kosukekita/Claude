...
3. **承認後、動画を生成する。参照人物があるかで2経路を選ぶ（★2026-07-14 ユーザー確定：同一人物・新シーンは経路Aを既定に）。**
   - **★経路A（推奨・既定）＝ 参照人物1枚＋テキスト絵コンテから reference-to-video で"直接"生成（キーフレーム画像を挟まない）**: 本人を別の場所・別の状況で動かす動画（例: 本人が仕事→スパでマッサージ）は、中間のキーフレーム画像を作らず、**各ショットを r2v で「reference画像＋そのショットのテキスト」から直接生成**するのが最適（キーフレーム→i2vの"つぎはぎ感"と工数を省ける）。モデルは用途で選ぶ: SFW/水着程度＝AtlasCloud Seedance `.../reference-to-video`（`cloud_atlascloud.py video` に参照画像を渡す・1〜3枚）／ NSFW寄り＝wan-2.7 reference系 or ローカル HunyuanCustom（`gen_hunyuan_custom.py`）。**★注意**: r2v は「手で胸を揉む」等の複雑な相互作用では破綻しやすい（[[hunyuancustom-r2v-nogo]]）。破綻したら経路Bにフォールバック。
   ...
   - **ショット間のカット（場面転換）は、どちらの経路でも各ショットを別々に生成して `ffmpeg concat` で繋ぐ**（カットは編集で作る）。
