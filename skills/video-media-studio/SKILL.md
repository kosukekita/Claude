...
- **★動画アスペクト比の既定＝9:16縦（2026-07-21 ユーザー確定）**: ユーザーが比率を指定しない動画生成は **`ratio: "9:16"`（縦）で作る**。`adaptive`（参照/開始画像の比率追従）や 3:4 を勝手に選ばない。理由: 主用途が IG Reels 等の縦型SNS配信で、**75Gravity の投稿前チェッカは 9:16 を厳密検査**（`width*16==height*9`）するため、3:4 で本番を作ると投稿段階でパッド/クロップ/再生成が必要になる（実機 2026-07-21: 3:4 の1080p本番がチェッカ不通過→9:16で再生成）。横型・スクエア・参照比率追従はユーザーが明示したときだけ。
  - **★「先の動画と同じ構図で」＝同じ"比率"ではない（ユーザー確定 2026-07-26）**: 参照/先行動画の**構図（寄り・被写体配置）を引き継ぐ指示**を、その参照の**アスペクト比を引き継ぐ許可**と解釈しない。**ユーザーが比率を明示していない限り、参照が4:5等でも成果物は9:16で作る**。実機 2026-07-26: 「先の動画（4:5）と同じ構図で」を4:5で作り、IGチェッカ不通過。加えてその4:5自体が「4:5寄りの開始フレームをVeo9:16に入れ黒帯を切った副産物」だった＝**i2vで9:16納品なら開始フレームを最初から9:16構図で作り、Veoをネイティブ9:16で回して黒帯を出さない**（黒帯を切って4:5化する運用に逃げない）。
  - **★非9:16を納品する前に必ず自己申告**: 成果物が9:16でないとき（16:9・1:1・4:5・参照追従など）は、最終化の前に「これは9:16でなく〇〇です。IGチェッカは通りません。よいですか？」と一言確認してから確定する。黙って非9:16で仕上げない。
...
2b. **★ゲート2: 演出ボード（鉛筆ラフ＋矢印）を生成し、ユーザー承認を得る。承認前に本番動画を生成しない**（2026-07-17 ユーザー確定）。
   ...
   - **★人物は「無個性プロキシ」で描く（ユーザー確定 2026-07-28・Codex 相談済み）**: ボードに描いた顔・髪型・服は**生成映像の人物に引っ張られる**ので、同一性情報をボードから物理的に消す。**頭は卵形＋十字の補助線だけ（顔立ちなし）／髪は1本も描かない（髪の動きは RED 矢印＋薄いグレーの点線で示す）／服はグレーのシルエット包絡線のみ（柄・ボタン・襟デザインなし）**。木製ドール風マネキンは木目・球体関節が混入するので不可、普通の人物ラフが最も汚染リスクが高い。実機 2026-07-28: 「髪を解く」カットでモデルが毛流れ・前髪を描き、ユーザーに「02に髪型が入ってる」と指摘された（`NO HAIR ANYWHERE ... completely bald and blank in all panels` を明示して解消）。
   - **★ボードは2枚書き出す（同 2026-07-28）**: **人間確認版**（カット番号・秒数・色矢印・注記あり＝ゲート2の承認用）と、**生成投入版（クリーン）**（矢印・文字・数字・注記・枠マークを全部除去し、構図/ポーズ/カメラ/光だけ）。**動画生成の参照に渡すのはクリーン版**（否定文だけでは漏れを防ぎ切れない＝入力から不要情報を物理的に消す方が強い。Veo はスタイル画像を画風に反映し、Seedance は構図・カメラ・動作・ビジュアルを横断参照するため）。矢印は焼き込まず、動きはプロンプト文章に変換して渡す。
   - 生成は **Codex（GPT Image）** or **Higgsfield `nano_banana_pro`**（実機 2026-07-28 で無個性プロキシ＋クリーン版の生成を確認）。実機検証済みのプロンプト定型・罠は `reference/directing-board.md`。
   - **ボードは i2v/r2v の"人物同一性"の正本ではない**（同一性は参照写真が担う）。ボードが担うのは構図・動き・カメラ・光・VFX の設計。だから人物は**無個性プロキシ**でよい（「ラフでよい」を"顔や髪を適当に描いてよい"と読まない＝描いた特徴は汚染源になる）。
...
- **★i2v は「開始フレーム（1フレーム目）をユーザー承認してから」後続生成に進む（ユーザー確定 2026-07-26）**: i2v は開始フレームが人物・衣装・構図・設定（例: ホテルで座っている等）を確定させ、後戻りできない（キーフレーム承認ルールと同旨）。**開始フレームを作ったら、動画本体・余韻クリップ・字幕など後続を生成する前に必ずユーザーに提示して承認を得る。** 承認前に余韻等の後続を先に作らない（実機 2026-07-26: 承認前に余韻を生成して手戻り）。開始フレームは元動画/先行動画と**同じ設定・構図（場所・ポーズ・服・体型ペルソナ cup/body）を引き継ぐ**——人物 ref だけで作ると場所や座り姿勢が抜けるので、場所を維持するならホテル室内 ref も渡し「座って」等を明示する。
