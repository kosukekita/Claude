...
2. **ストーリーボードを"1枚の画像"として作る**（`storyboard_*.png`）。各コマ＝各クリップの**開始/終了キーフレーム**（記号・矢印で誤魔化さず、そのまま開始/終了画像に使える"実画像"にする）。同 basename の `storyboard_*.txt` に、各クリップの**尺(秒)・開始コマ・終了コマ・内容・カメラ**を紐づける。
