...
- **1本の動画から別カメラアングル/マルチカメラ映像**（同じシーンを別視点で再撮影・novel-view）→ **LTX-2.3 CrossView IC-LoRA**（`reference/ltx-crossview-multicam.md`）。参照動画＋固定語彙プロンプト `crossview. new camera angle: {方位}, {高さ}, {距離}.`（全63通りは `reference/crossview_captions_all_63.txt`）で v2v。実行は ComfyUI 経路（IC-LoRA本体は `/data/kita/ComfyUI/models/loras/` に取得済み。基盤スタック~50GB＋custom node は要導入）。
