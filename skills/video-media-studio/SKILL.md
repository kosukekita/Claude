...
2. **ストーリーボードを"1クリップにつき1枚の画像"として作る**（`storyboard_<shot>_NN.png`）。1枚に入れてよいのは、そのクリップの**開始/終了キーフレーム2枚だけ**（記号・矢印で誤魔化さず、そのまま開始/終了画像に使える"実画像"にする）。同 basename の `storyboard_<shot>_NN.txt` に、**尺(秒)・開始コマ・終了コマ・内容・カメラ**を紐づける。
...
5. **1ストーリーボード＝1連続ショット。カメラのカット・場面転換・時間ジャンプを1回でも内包しない。** カットが変わったら必ず**別ファイルの別ストーリーボード・別クリップ**にし、最後に`ffmpeg concat`で繋ぐ。★複数カットが同じ作品/同じ流れでも、「承認しやすい」「1つの連続した流れだから」を理由に**1枚にまとめない**（＝letter vs spiritの禁止。**カットを1枚に束ねた瞬間このルールを破っている**）。**機械的自己チェック**: 1つのストーリーボード(txt)は「開始＋終了の2キーフレーム＝1クリップ」だけを含む。2組目のシーン/場面が出てきたら、それは別ストーリーボードのサイン。命名 `storyboard_<shot>_NN.(png|txt)`（shot毎に1ファイル）。
