...
5. **カット（場面転換）が入る所で区切り、"別動画・別ストーリーボード"にする**（1ストーリーボード＝1連続ショット）。
...
**Red flags（どれも「STOP・先に正しいストーリーボードを作れ」）**: 「0.6s刻みのi2v」「start/end無しでi2v」「1本にカットを詰める」「freeze-frameで尺を埋める」「承認をあとで取る」。
