...
### リアル写真の自然化プロンプト（AIっぽさを消す・提案して使う）

**ユーザーがリアル/実写風/フォトリアル画像を求めたとき、または「AIっぽい」「肌がツルツルすぎ」「照明が作り物っぽい」「背景が浮く」「SNSでバレたくない」と言ったときは、`reference/realism-naturalization-prompts.md` の自然化プロンプト（全30個・6カテゴリ）から該当するものを選んで提案する。** 人物の肌・手・表情、照明の整合性、全体のリアル感が大きく向上する実証済みの知見（出典は同ファイル）。

- **既定の提案 = スターター3個**: ①「SNSに実在しそうな自然な写真にしてください」②「過度な加工感はなくしてください」㉚「自然さを最優先してください」。症状が具体的なら同ファイルの「症状→カテゴリ対応表」で 2〜3 個追加。
- **勝手に足さない**: 6要素テンプレートの確認ルールと同じ。「この自然化プロンプトを足しませんか」と提案し、ユーザーの OK 後にプロンプト末尾へ仕上げ層として追記する。30個を一括投入しない（薄まる）。
...
- **(A) ペルソナ新規 SFW シート = 参照画像なし（t2i）**: 「新しいSFWのキャラシートを作って」はこれ。**ペルソナ固定ルール（年齢25〜35歳・顔75点・胸Fカップ）以外の特徴は自動設定してよい**（確認不要・6要素フローの明示的例外。2026-07-08ユーザー確定）。実写調既定・フル構成（表情込み）。Codex text-only 第一（**Fカップ指定は拒否されうる→婉曲→Grok CLI 日本語**の順）。
