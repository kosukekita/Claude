...
| (2b) **キービジュアル生成** | **広告/Web/スライド/SNSサムネ用の非人物キービジュアル**（Xサムネイル・LPファーストビュー・スライド表紙・Web背景等） | **`reference/key-visual-director.md` が既定フロー・必読**（K0 ブリーフ2項目ゲート → K1 3方向A/B/C生成・比較・選択 → K2 改善版＋プロ級監査。2026-07-21 ユーザー確定）。読まずに着手した時点で工程未実施扱い |
...
>
> **★適用範囲の分離（2026-07-21 ユーザー確定）: この1本ルールは「リアル人物画像・動画生成」の規律。** 広告/Web/スライド用の**非人物キービジュアル**（(2b)・`reference/key-visual-director.md`）は明確に分離し、**3方向 A/B/C 各1枚＋改善版1枚＝計4枚が既定**（ただし並列量産はせず1枚ずつ順次生成・各軽検品。計4枚を超えるのはユーザーの本数明示時のみ）。人物が主役として登場する画像・動画には従来どおり1本ルールが効く。
