...
>
> **★例外（2026-07-17 ユーザー確定）: 演出ボード（StoryBoard＝鉛筆ラフ＋矢印）は「設計図」なので文字を入れる**——カット番号・秒数ラベル・緑のフレーミング注記・黒のショット注記・下端の凡例ストリップは**必須**。ボードに上の negative を入れない。この恒久ルールが効くのは**納品する映像・写真**（キーフレーム画像を含む）であって、承認用の設計図ではない。同様にキーフレームのシートには秒数ラベルだけ入れてよい。
...
2b. **★ゲート2: 演出ボード（鉛筆ラフ＋矢印）を生成し、ユーザー承認を得る。承認前に本番動画を生成しない**（2026-07-17 ユーザー確定）。
   ...
   - **1枚のシート画像として生成**（グリッド）。各パネルに**カット番号・秒数ラベル・凡例どおりの色矢印・黒のショット注記**を焼き込み、**シート下端に凡例ストリップ**を入れる。
