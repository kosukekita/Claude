...
## 使い方(適用の指針)
- **★既定で"最もシネマティックな"カメラワークを選ぶ（安全なStatic/固定に逃げない）**。ショットの意図に最も映える動きを下の46技法から選んで使う（ユーザー確定 2026-07-12）。指示が無くても、静止でなく意図に合う映像的な動きを積極採用する。目安: **カメラへ向かってくる→reverse tracking**（人物は大きいまま背景が流れる＝迫力。単なる遠→近のStaticより映える） / **人物リビール→crane up・pedestal up** / **周囲を見せる/バレットタイム風→arc・orbit** / **スケール感→crane・drone** / **緊張/めまい→dolly zoom** / **スポーツ感→low tracking**。ただし下記のi2v制約（1枚目に無い要素へは動けない・周回は周囲の空間が要る等）は守り、破綻するなら一段控えめな動きに落とす。
