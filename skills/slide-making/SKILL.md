...
2. **フェーズ1〜6 を既定で実行**: 曖昧さの発見 → 中心概念 → 構成案（9部の物語構造） → 視覚設計 → 3方向デザイン案 → 反対監査
   ...
   省略はユーザーが**工程名を特定して**指示した時のみ（「急ぎ」「シンプルに」は省略指示ではない — `§3` 冒頭）。
...
6. **AskUserQuestion で合意**: 質問は束ねて最小往復（R0 出力形式＋ブリーフ7項目 → R1 中心概念＋全体構成 → R2 3案選択＋各スライドの構造/図種 → R3 反対監査の結果＋設計図の最終承認）。
   往復の圧縮＝**各ラウンド内で束ねる**こと。R0〜R3 のラウンド自体は統合しない（特に **R2 と R3 の統合禁止** — 監査は選ばれた案に対して行う）。
   **承認前に HTML/PPTX 実装を始めない。**

R3 承認後にスライド追加・構成変更が生じたら、該当分の設計図ブロックを追記・更新し、**変更分への反対監査を再実施してから**実装する
（ブリーフは差分のみ確認）。軽微修正＝当該スライドの GOAL・So What・構造型・図種・数字の種類・枚数・順序の**いずれも変えない**
文言・数値の修正のみ（設計図の範囲内で直接編集可。これらに触れる変更は構成変更＝差分監査＋承認必須）。
この増分パスを使えるのは**本セッションで R3 承認済みの設計図が現存する場合のみ**。承認済み設計図が無い既存デッキの改修依頼は、
現物から設計図を復元したうえで R0（差分ブリーフ可）→ 変更範囲に応じた R1/R2 → R3（変更分監査）を通す。
...
フォント: `design-rules.md §2` が正（`'Noto Sans JP'` 基点のスタック）。**Iron Law: メイン+アクセントの合計 5% 以下（目標ではなく上限）。**
