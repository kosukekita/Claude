...
## STEP 1 — 設計図（Markdown）を作って合意（実装の前に必ず）

**いきなりスライドを作らない。** STEP 0 の直後・コンテンツ設計の前に、**Markdown で「設計図」を起こし
AskUserQuestion で構成・構造・グラフ形式を合意してから** HTML/PPTX 実装に入る。詳細は `references/design-planning.md`。

1. **入力3点ゲート**: 着手前に `① 読み手は誰か ② 一番言いたいこと ③ 手元の数字` を確認。
   1つでも欠けたら先に質問して埋める（揃うまで設計図を書かない）。読み手で粒度を変える（経営層=結論・効果先出し／実務者=手順・根拠厚め）。
2. **設計図 Markdown を起こす**: 1スライド=1ブロックで `# 言い切りタイトル(15字以内) / > GOAL: / 構造型 / 軸 / 図種＋数字の種類 / 配置 / → だから(So What)` を埋める。
   `GOAL` か `So What` が書けないスライドは作らない。
3. **構造型を1つ当てる**: 構造6型（順番／対比／仕分け／問題打ち手効果／因果／結論先出し）から**1枚1つ**。詰める軸は2つまで（2つ言いたければ2枚に分割）。
4. **数字のあるスライドはグラフを逆算で決める**: `主張→必要な数字→図種` の順。**選んだ図種が出力パスで実装可能か確認**
   （ファネル・サンキー・ヒートマップ・モザイク等の Chart.js 標準外と PPTX の全グラフは `parts/*.png` に画像化）。
5. **AskUserQuestion で合意**: 質問は束ねて最小往復（R0 出力形式＋入力3点 → R1 全体構成 → R2 各スライドの構造/図種 → R3 設計図の最終承認）。
   **承認前に HTML/PPTX 実装を始めない。**

> 設計図フェーズで決めるのは「何を・どの順で・どの型で・どの図で」。見た目（配色・余白・強調・脱文章）は決めない（既存「コンテンツ設計」「デザインシステム」が正）。
...
| 入力3点（読み手・主張・数字）を聞かず着手 | 揃うまで設計図を書かない。欠けたら先に質問 |
...
- `references/design-planning.md` — **STEP 1 設計図フェーズ**（入力3点・設計図スキーマ・構造6型/グラフ12選の選定表・見出し言い切り）
