...
> ⚠️ **言語ポリシー（実機検証済み・最重要）: ユーザーが日本語で書いたプロンプトは、英訳せず日本語のまま grok に渡す。** Grok のヘッドレス実行では、英語の直接的な身体・露出表現（例 `busty`, `shirtless wearing boxer briefs`）がコンテンツフィルタに当たり、**エラーも返さず空応答で無言終了する**（画像が一切生成されない）。ところが**同じ内容を日本語のまま（「胸の大きい」「上半身裸でボクサーパンツ」）渡すと普通に生成できる**ことを実機で確認（2026-06-23）。切り分け実績: テキスト応答(PONG)✅ / `image_gen`英語「red apple」✅ / `image_gen`英語の人物NSFW❌無言 / **`image_gen`日本語の同一人物NSFW✅成功**。したがって**翻訳は『整形』ではなく改悪**になりうる。日本語プロンプトはそのまま渡し、補う要素（カメラ・尺・比率）だけ日本語で足す。英語が必要なのは元から英語で来た場合のみ。
