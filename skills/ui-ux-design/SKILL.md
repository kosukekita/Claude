...
5. 参照ファイルは必要なものだけ読む。**ただし新規LP と判定した場合は `references/lp-direct-response.md`、新規Webサイトと判定した場合は `references/design-orchestrator.md` が必読（このルールの例外）。読まずに着手した時点で工程未実施として扱う。**

## RULE PRECEDENCE

ルールが競合する場合は以下の優先順位に従う：

1. ユーザーの明示的な指定（＝具体値の指定。「お任せ」「いい感じに」等の包括的委任は含まない）
...
> この優先順位は**デザイン上の決定（値）の競合解決**にのみ適用する。工程（lp-direct-response の L0〜L4・フェーズ1〜6、
> および design-orchestrator の W0〜W4・■1〜■8）の実施義務はこの表の対象外であり、
> 省略は各正本 §3 の「工程名を特定した省略指示」によってのみ可能。
> 「すぐ作って」等の速度要求は precedence 1 の明示指定ではなく、各正本 §1 の通知型ルールで扱う。
...
| タスク種別 | 判断基準 | 処理フロー |
|---|---|---|
| **新規LP / セールスページ（1ページ・行動転換特化）** | 「LPを作って」「セールスページ」「成約率を上げたい」「オプトインページ」等（例示）。**成果物で判定**: 単一ページで特定の行動（購入/登録/相談/資料請求等）への転換を目的とするページ（ペライチ・キャンペーンページ・既存サイトへの追加1枚を含む）。LPかサイトか迷ったら着手前に1問で確認 | **`references/lp-direct-response.md` が既定フロー**: L0 ブリーフ7項目ゲート → フェーズ1〜6（認識変化/中心オブジェクト/ファーストビュー3案/心理10段階構成/視覚設計/反証）→ L4 承認後に実装 → 品質チェック |
| **新規Webサイト（複数ページ: コーポレート・サービス・採用・ポートフォリオ・メディア等）** | 「サイトを作って」「ホームページ」「リニューアル」等（例示）。**成果物で判定**: 複数ページ構成の新規サイト、行動転換特化でない新規ページ（ティザー・特設等）、トップページのみの作り直し・情報設計/ページ構成/デザインシステムのいずれかを変えるリニューアル。既存UI修正行と迷う場合は分類前に `design-orchestrator.md` の適用範囲節を読む | **`references/design-orchestrator.md` が既定フロー**: W0 ブリーフ6項目ゲート → ■1〜■8（要件構造化/中心視覚概念/3案/情報設計/ページ設計/デザインシステム/実装構成/反対監査）→ W4 承認後に実装 → 品質チェック |
| **新規 Dashboard / アプリ画面UI**（サイト以外の新規） | 「ダッシュボード作って」「管理画面」「アプリのUI」。**公開Webで不特定の訪問者に閲覧・訴求するページ（ホーム/紹介/料金/採用等）は、アプリ画面と同居していてもサイト/LP成果物として上の行で扱う。迷ったら上の行** | 要件分析 → デザインシステム定義 → 実装 → 品質チェック |
...
> **行の優先**: 新規LP/サイトの作成でスタイル明示指定（Swiss/雑誌風/Nothing 等）がある場合は**上2行（LP/サイトのフロー）が優先** —
> swiss-modernism.md / nothing-design.md は各フローの3案工程（フェーズ3/■3）以降のスタイル規律として内部適用する。
> Swiss 行は既存物のグリッド検証・identity/wayfinding 等、LP/サイト新規作成**以外**に適用する。
...
> **新規LP は `references/lp-direct-response.md` の L0〜L4、新規Webサイトは `references/design-orchestrator.md` の
> W0〜W4 が既定フロー**（ユーザー確定 2026-07-21）。LP はブリーフ7項目（商品/価格/対象者/悩み/結果/根拠/CTA）、
> サイトはブリーフ6項目（種類/事業/利用者/行動/印象/掲載情報）の**未指定分を着手前に必ず質問**し、
> Step 1〜4 はそのフローの内部で使う（Step 2 の5要素確定は 3案選択後。最終承認前に実装しない。
...
### Step 2: デザイン方向の決定
新規設計時のみ以下の5要素を決定して提示（**新規LP では L2（lp-direct-response.md 参照）、新規Webサイトでは W2（design-orchestrator.md 参照）の3案選択後に確定**）：
...
> 質問するか `〔要差し替え〕` プレースホルダーで明示する（`lp-direct-response.md §1 #2/#6`・`design-orchestrator.md §1 #6`）。
...
7. **出所ゲート**: スコア・配点・スペック数値・「当サイト調べ」等の出典表示は、ユーザー提供の実データがある場合のみ。
   未提供は質問するか `〔要差し替え〕`（`lp-direct-response.md §1 #6`・`design-orchestrator.md §1 #6`・【禁止事項】と連動）。
...
- `references/lp-direct-response.md` — **新規LP/セールスページの既定フロー正本**（ブリーフ7項目ゲート・フェーズ1〜6プロンプト全文・禁止事項・L0〜L4統合マップ・合理化反論表）
- `references/design-orchestrator.md` — **新規Webサイト（複数ページ）の既定フロー正本**（ブリーフ6項目ゲート・■1〜■8プロンプト全文・W0〜W4統合マップ・合理化反論表）
...
- `references/third-party-notices.md` — 取り込み元MITライセンスの著作権・許諾表示
