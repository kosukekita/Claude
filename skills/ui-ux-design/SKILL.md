...
> この優先順位は**デザイン上の決定（値）の競合解決**にのみ適用する。工程（design-orchestrator の W0〜W4・■1〜■8）の
> 実施義務はこの表の対象外であり、省略は同 §3 の「工程名を特定した省略指示」によってのみ可能。
> 「すぐ作って」等の速度要求は precedence 1 の明示指定ではなく、同 §1 の通知型ルールで扱う。
