...
5. 参照ファイルは必要なものだけ読む。**ただし新規Webサイト/LP と判定した場合、`references/design-orchestrator.md` は必読（このルールの例外）。読まずに着手した時点で工程未実施として扱う。**
...
| タスク種別 | 判断基準 | 処理フロー |
|---|---|---|
| **新規Webサイト / LP（コーポレート・サービス・採用・ポートフォリオ・メディア等）** | 「サイトを作って」「ホームページ」「LPを作って」「リニューアル」等（例示）。**フレーズ一致でなく成果物で判定**: 新規のWebページ/サイトを作る依頼（ペライチ・特設/キャンペーンページ・ティザー・既存サイトへの1ページ追加・**トップページのみの作り直し・情報設計/ページ構成/デザインシステムのいずれかを変えるリニューアル**を含む）。既存UI修正行と迷う場合は分類前に `design-orchestrator.md` の適用範囲節を読む | **`references/design-orchestrator.md` が既定フロー**: W0 ブリーフ6項目ゲート → ■1〜■8（要件構造化/中心視覚概念/3案/情報設計/ページ設計/デザインシステム/実装構成/反対監査）→ W4 承認後に実装 → 品質チェック |
...
> **行の優先**: 新規サイト/LP の作成でスタイル明示指定（Swiss/雑誌風/Nothing 等）がある場合は**1行目（orchestrator フロー）が優先** —
> swiss-modernism.md / nothing-design.md は ■3 以降のスタイル規律として内部適用する。Swiss 行は既存物のグリッド検証・
> identity/wayfinding 等、サイト新規作成**以外**に適用する。
